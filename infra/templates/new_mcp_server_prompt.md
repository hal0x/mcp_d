...
Требования к коду
- По умолчанию используй `mcp.server.fastmcp.FastMCP` с декораторами `@mcp.tool()` для 90% случаев.
- Используй низкоуровневый `mcp.server.Server` с `@server.list_tools()` и `@server.call_tool()` только если нужен:
  - Кастомный диспатчинг инструментов (ручная маршрутизация)
  - Единое форматирование ответов
  - Динамическое построение списка инструментов
...
- Простые FastMCP‑сценарии: `shell-mcp`, `supervisor-mcp`, `policy-mcp`, `learning-mcp`, `backtesting-mcp`.
- Низкоуровневая реализация MCP Server для кастомной логики: `binance-mcp` (кастомный диспатчинг 50+ инструментов), `tradingview-mcp` (интеграция legacy-кода), `memory-mcp` (ленивая инициализация).

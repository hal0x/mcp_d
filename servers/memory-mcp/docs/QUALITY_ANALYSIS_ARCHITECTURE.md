...
Система анализа качества представляет собой отдельный модуль `quality_analyzer`, который работает по принципу **реального тестирования**: извлекает информацию из исходных файлов чатов, выполняет поиск через Memory MCP, и оценивает релевантность результатов с помощью LM Studio или Ollama.

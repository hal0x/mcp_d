# Система анализа качества индексации memory_mcp

## Общая архитектура

Система анализа качества представляет собой отдельный модуль `quality_analyzer`, который работает по принципу **реального тестирования**: извлекает информацию из исходных файлов чатов, выполняет поиск через memory_mcp, и оценивает релевантность результатов с помощью Ollama.
...
### 2. RelevanceAnalyzer
**Функция**: Анализ релевантности результатов через Ollama API

**Процесс работы**:
- Отправляет прямые HTTP запросы к Ollama API
...
2. **Поиск через memory_mcp**: выполнение каждого запроса через существующую систему поиска
3. **Анализ релевантности**: оценка результатов через Ollama с адаптивными промптами
...
### Этап 4: Отчетность
1. **Автоматические рекомендации**: Ollama предлагает конкретные улучшения
...
Эта система позволит постоянно мониторить и улучшать качество индексации и поиска в memory_mcp, обеспечивая высокое качество результатов для пользователей.

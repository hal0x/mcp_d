    ...
    async def _generate_single_summary(
        ...
        for attempt in range(2):
            try:
                ...
                # ВСЕГДА используем streaming для предотвращения отключения клиента
                # Streaming позволяет клиенту получать данные во время генерации
                # Это критично для длительных запросов, так как LM Studio Server может отключить клиента
                # если не получает активности в течение 5 минут
                use_streaming = True  # Всегда используем streaming для генерации саммари

...
class SessionSummarizer:
    ...
    async def summarize_batch_sessions(
        ...
        # Получаем контекст из предыдущих сессий
        previous_context = self.context_manager.get_previous_context(
        ...
        chat_mode = self._detect_chat_mode(sessions[0].get("messages", []) if sessions else [])
                ...
                for i, (session, summary_data) in enumerate(zip(sessions, detailed_summaries)):
                    ...
                    summary_dict = {
                        ...
                        "meta": {
                            ...
                            "profile": self._map_profile(chat_mode, session.get("messages", [])),
                        },

...
2. **Общие нормализаторы аргументов и конфигурации**
   ...
   - Разработать `utils.paths.resolve_project_path()` / `get_data_dir()` и заменить ручной поиск `pyproject.toml`, чтобы все сервисы одинаково определяли пути и `MEMORY_DB_PATH`.

...
class VolumeProfileScanner(BaseScanner):
    ...
    def __init__(
        ...
        self.strategy_name = "volume_profile"
    ...
    async def run(self, symbols: Sequence[str], profile: str) -> list[ScannerSignal]:
        ...
        for symbol in symbols:
            cache_key = make_cache_key("volume_profile:signal", [symbol, profile])
    ...
    def _evaluate_klines(self, klines: Sequence[dict]) -> VolumeProfileEvaluation | None:
        ...
        price_levels = np.linspace(lows.min(), highs.max(), num=50)
        hist, bin_edges = np.histogram(closes, bins=price_levels, weights=volumes)
        if not np.any(hist):
            return None
        poc_index = int(np.argmax(hist))
        poc = float((bin_edges[poc_index] + bin_edges[poc_index + 1]) / 2)
        cumulative = np.cumsum(hist) / hist.sum()

        value_area_low, value_area_high = poc, poc
        for idx in range(len(hist)):
            val = cumulative[idx]
            if val <= 0.15:
                value_area_low = float((bin_edges[idx] + bin_edges[idx + 1]) / 2)
            if val >= 0.85 and value_area_high == poc:
                value_area_high = float((bin_edges[idx] + bin_edges[idx + 1]) / 2)
        ...
        if last_close > value_area_high:
            ...
            reasons.append(f"Price above value area high {value_area_high:.2f}")
            confidence += 10
        elif last_close < value_area_low:
            direction = SignalDirection.SHORT
            reasons.append(f"Price below value area low {value_area_low:.2f}")
            confidence += 10
        else:
            return None
        ...
        reasons.append(f"POC {poc:.2f}")
        reasons.append(f"ATR {last_atr:.2f}")

        return VolumeProfileEvaluation(
            direction=direction,

...
- **Паттерны**: Стандартный MCP Server с кастомным диспатчингом

##### Почему используется стандартный MCP Server

Binance MCP использует низкоуровневый `mcp.server.Server` вместо FastMCP по следующим причинам:

1. **Ручной диспатчинг 50+ инструментов**: Сервер содержит более 50 торговых инструментов, которые обрабатываются через большой `if/elif` блок в функции `call_tool()`. Это позволяет:
   - Централизованно управлять маршрутизацией всех инструментов
   - Легко добавлять новые инструменты в едином месте
   - Обеспечивать единообразную обработку ошибок
...
3. **Кастомная обработка ошибок**: Функция `_format_error_message()` обеспечивает единообразные сообщения об ошибках с префиксом "Ошибка:" для всех инструментов.

4. **Динамическое построение списка инструментов**: Функция `list_tools()` программно создает список всех доступных инструментов с их схемами, что позволяет легко добавлять или изменять инструменты без изменения структуры кода.
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> ToolResponse:
    ...
    if name == "get_account_info":
        result = await AccountService.get_account_info()
        return _format_tool_response(result.model_dump())
    # ... 50+ других инструментов
```
